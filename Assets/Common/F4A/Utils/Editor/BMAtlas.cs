...
public class BMAtlas
{
    //[MenuItem("Assets/SplitImageFromAtlas/Create from text file")]
    ...
    [MenuItem("Assets/SplitImageFromAtlas/Create from texture meta")]
    public static void ImportBitmapFontFromMeta()
    {
        ...
        if (selectedObjects.Length > 0 && selectedObjects[0] is Texture2D)
        {
            ...
            string selectionPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(selectedObjects[0]));
            System.IO.Directory.CreateDirectory(selectionPath + "/" + selectedObjects[0].name);
            ...
            while (index < lines.Length)
            {
                ...
                if (line.StartsWith("- serializedVersion"))
                {
                    ...
                    try
                    {
                        //var pivot = Newtonsoft.Json.JsonConvert.DeserializeAnonymousType(strPivot, pivotDefinition);
                        //px = (float)pivot.x;
                        //py = (float)pivot.y;
                        px = 0.5f;
                        py = 0.5f;
