	...
	public abstract class DailyRewardsCore<T> : MonoBehaviour where T : Component
	{
		...
		public IEnumerator InitializeDate()
		{
            if (useWorldClockApi)
			{
               ...
               //nityWebRequest www = UnityWebRequest.Get(worldClockUrl);
             // www.SendWebRequest();
#else
				...
				if (false)
				{
					...
					{
						isErrorConnect = true;
						//bug.LogError("Error Loading World Clock:" + www.error);

					//rrorMessage = www.error;
					}
				}
				else
				{
#if UNITY_2018_3_OR_NEWER
                   //ar clockJson = www.downloadHandler.text;
#else
                    var clockJson = www.text;
#endif

                    // Loads the world clock from JSON
                   //orldClock = JsonUtility.FromJson<WorldClock>(clockJson);
