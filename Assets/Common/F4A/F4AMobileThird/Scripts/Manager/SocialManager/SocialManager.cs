...
#endif
            DMCStoreGameInfo mystore = StoreGameInfos.Where(st => st.StorePublishType == EStorePublishType.GooglePlayStore).FirstOrDefault();
            return mystore.UrlGameOnStore;
        }
...
#region OTHERS
	    ...
	    {
	       //MCMobileUtils.OpenURL(socialSettingInfo.GetUrlGameOnStore());
        }
    ...
    //      DMCMobileUtils.OpenURL(socialSettingInfo.linkFacebookPage);
     ...
     //     DMCMobileUtils.OpenURL(socialSettingInfo.linkTwitter);
       ...
       //   DMCMobileUtils.OpenURL(url);
        ...
        public void SendMail(string body = "", string subject = "", string cc = "", string bcc = "")
        {
            string email = socialSettingInfo.GetEmailContact();
     //     if(string.IsNullOrEmpty(subject)) subject = EscapeURL(BuildManager.Instance.GetGameName() + " [" + BuildManager.Instance.GetVersion() + "] Support");
      //    if (string.IsNullOrEmpty(body)) body = EscapeURL("Feedback:");
            var url = "mailto:" + email + "?subject=" + subject + "&body=" + body;
            if (!string.IsNullOrEmpty(cc)) url += "&cc=" + cc;
            if (!string.IsNullOrEmpty(bcc)) url += "&bcc=" + bcc;
     //     DMCMobileUtils.OpenURL(url);
        }
