...
using UnityEngine.Purchasing.Security;
...
#endif
	{
		...
		public EStatusBuyIAP BuyProductByID(string productId)
		{
            ...
            {
                DOVirtual.DelayedCall(0.25f, delegate { OnBuyPurchaseSuccessed?.Invoke(productId, true, string.Empty); });
...
#region IStoreListener
        ...
        public bool ProductAvailableToPurchase(string id)
        {
            var product = GetProductInfoById(id);
            return ProductAvailableToPurchase(product);
        }
