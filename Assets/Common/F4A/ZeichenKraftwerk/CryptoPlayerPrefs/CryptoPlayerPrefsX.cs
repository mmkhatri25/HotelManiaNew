// Crypto adjusted ArrayPrefs2 v 1.3
// We (Zeichenkraftwerk) just added encryption, the array stuff comes out of the wiki (Source: http://wiki.unity3d.com/index.php/ArrayPrefs2).
// Thanks to Eric Haines (http://www.starscenesoftware.com/)
...
public class CryptoPlayerPrefsX
{
    ...
    public static bool SetBoolArray(String key, bool[] boolArray)
    {
        ...
        ((ICollection)bits).CopyTo(bytes, 5);
        Initialize();
    ...
    public static bool[] GetBoolArray(String key)
    {
        if (CPlayerPrefs.HasKey(key))
        {
            var bytes = System.Convert.FromBase64String(CPlayerPrefs.GetString(key));
            if (bytes.Length < 6)
            {
            ...
            ICollection bitCol = (ICollection)bits;
            var boolArray = new bool[bitCol.Count];
            bitCol.CopyTo(boolArray, 0);
    ...
    /// <summary>
    /// Returns the value corresponding to key in the preference file if it exists.
    /// If it doesn't exist, it will return an array of size {defaultSize} with value {defaultValue}.
    /// </summary>
    ...
    public static bool SetStringArray(String key, String[] stringArray)
    {
        ...
        try
        {
            CPlayerPrefs.SetString(key, System.Convert.ToBase64String(bytes) + "|" + String.Join("", stringArray));
        }
        catch
        {
            return false;
    ...
    public static String[] GetStringArray(String key)
    {
        if (CPlayerPrefs.HasKey(key))
        {
            var completeString = CPlayerPrefs.GetString(key);
    ...
    private static bool SaveBytes(String key, byte[] bytes)
    {
        try
        {
            CPlayerPrefs.SetString(key, System.Convert.ToBase64String(bytes));
        }
        catch
        {
            return false;
